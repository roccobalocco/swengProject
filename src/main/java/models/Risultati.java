...
public class Risultati {
    ...
    public String toString(){
        ...
        if(isClassica) {
            content.append(c.descrizione + "\n");
            content.append("\n");
            content.append("Scadenza: ").append(c.getScadenza().toString());
            content.append("\n");
            ...
            content.append(r.descrizione + "\n");
            content.append("\n");
            content.append("Scadenza: ").append(r.getScadenza().toString());
            content.append("\n");
    ...
    public boolean printRisultati(String path) throws FileNotFoundException, DocumentException {
        ...
        if(isClassica) {
            PdfWriter.getInstance(document, new FileOutputStream(path + "/" + c.descrizione.replaceAll("[ -/'\"]", "_") +
                    c.getScadenza().toString().replaceAll("[ -/'\"]", "_") + ".pdf"));

            document.open();
            Font font = FontFactory.getFont(FontFactory.COURIER, 16, BaseColor.BLACK);
            StringBuilder content = new StringBuilder(c.descrizione + "\n");
            content.append("\n");
            content.append("Scadenza: ").append(c.getScadenza().toString());
            content.append("\n");
            content.append("Si: ").append(si).append("\n");
            ...
            content.append("Bianca: ").append(bianca).append("\n\n\n");
            content.append(vinceRef());
            Chunk chunk = new Chunk(content.toString(), font);

            document.add(chunk);
            document.close();
        }else{
            PdfWriter.getInstance(document, new FileOutputStream(path + "/" + r.descrizione.replaceAll("[ -/'\"]", "_") +
                    r.getScadenza().toString().replaceAll("[ -/'\"]", "_") + ".pdf"));

            document.open();
            Font font = FontFactory.getFont(FontFactory.COURIER, 16, BaseColor.BLACK);
            StringBuilder content = new StringBuilder(r.descrizione + "\n");
            content.append("\n");
            content.append("Scadenza: ").append(r.getScadenza().toString());
            content.append("\n");
            content.append(gpString());
            ...
            if(c.whichType() == 2)
                votiPersone.forEach((p, ii) -> {
                    if (p.getGruppo() == g.getId())
                        s.append("\t").append(p.toString()).append(" -- Preferenze secondo il calcolo del sistema: ").append(ii).append("\n");
                });
        });
        return s.toString();
    }
    ...
    private String vinceCla() {
        ...
        if(c.whichType() != 2){
            ...
            if(!c.isAssoluta() || hasWin(max))
                if(pari)
                    return "Non é possibile stabilire un vincitore a causa di una paritá tra: " + l.toString() + "\n\n";
                else
